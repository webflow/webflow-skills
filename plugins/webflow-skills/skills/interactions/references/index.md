...
| `[FLAG]`                | Availability depends on a feature flag that is **not visible in the payload**, so the same caller may be accepted or refused as rollout changes.                                                       | Attempt it and handle the refusal. Do **not** pre-emptively refuse.                                        |
