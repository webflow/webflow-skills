...
| Hardcoded colors/sizes | Expose as `props.Variant()` |
...
| CSS-in-JS not configured | styled-components/Emotion without decorator | Add globals.ts with decorator |
...
props: {
  ...
  badgeType: props.Variant({
    name: "Badge Type",
    options: ["sale", "new", "featured", "none"],
    defaultValue: "none"
  })
}
```
