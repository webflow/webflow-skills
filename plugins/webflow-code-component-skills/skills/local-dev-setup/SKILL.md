...
9. **Verify setup**:
   - Run the React project locally to test component renders
   - The only way to test in Webflow is to deploy with `npx webflow library share`
...
Then create your component files (.tsx, .webflow.tsx, .module.css) and deploy:
```bash
...
3. **Validate**: Use build-validate skill
4. **Deploy**: `npx webflow library share`
