...
6. **Apply TypeScript → Webflow prop type mapping**:
   ...
   | `"option1" \| "option2"` | `props.Variant()` | String literal unions |
   | `enum` | `props.Variant()` | Convert enum values to options array |
   | `{ href: string; ... }` | `props.Link()` | Link objects |
...
7. **Handle special cases**:
   ...
   **Optional props** - Always provide defaultValue:
   ```typescript
   // Original
   interface Props {
     title?: string;
...
declareComponent(ComponentName, {
  name: "ComponentName",
...
declareComponent(TestimonialCard, {
...
export const ProductCard = ({
  ...
  <div className={styles.card}>
    {productImage && (
      <img src={productImage.url} alt={productImage.alt || productName} />
    )}
...
- Image prop now uses Webflow's Image type (provides { url, alt })
```
